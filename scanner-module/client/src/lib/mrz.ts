...
function parseTD3(lines: string[]): MRZResult | null {
  ...
  if (!validateCheckDigit(line2.substring(0, 9), docCheckDigit)) warnings.push("doc_number_check_failed");

  const nationality = line2.substring(10, 13).replace(/</g, "");
  const birthRaw = line2.substring(13, 19);
  const birthCheckDigit = line2.substring(19, 20);
  if (!validateCheckDigit(birthRaw, birthCheckDigit)) warnings.push("birth_date_check_failed");

  const gender = parseMRZGender(line2.substring(20, 21));
  const expiryRaw = line2.substring(21, 27);
  const expiryCheckDigit = line2.substring(27, 28);
  if (!validateCheckDigit(expiryRaw, expiryCheckDigit)) warnings.push("expiry_date_check_failed");
  ...
  return {
    ...
    surname, givenNames, documentNumber: docNumber, nationality,
    dateOfBirth, gender, expiryDate, issuingCountry,
    confidence: Math.max(0, confidence), warnings,
  };
}
...
function parseTD1(lines: string[]): MRZResult | null {
  ...
  if (!validateCheckDigit(line1.substring(5, 14), docCheckDigit)) warnings.push("doc_number_check_failed");
  ...
  return {
    documentType, surname, givenNames, documentNumber: docNumber, nationality,
    dateOfBirth, gender, expiryDate, issuingCountry,
    confidence: Math.max(0, confidence), warnings,
  };
}
...
// MRVA — Visa Type A (2 lines x 44 chars)
...
// MRVB — Visa Type B (2 lines x 36 chars)
function parseMRVB(lines: string[]): MRZResult | null {
  ...
  const gender = parseMRZGender(line2.substring(7, 8) === "M" || line2.substring(7, 8) === "F" ? line2.substring(7, 8) : line2.substring(20, 21));
  const expiryRaw = line2.substring(21, 27);
  const expiryCheckDigit = line2.substring(27, 28);
  if (!validateCheckDigit(expiryRaw, expiryCheckDigit)) warnings.push("expiry_date_check_failed");
...
export function parseMRZ(rawText: string): MRZResult | null {
  const lines = rawText
    .split("\n")
    .map((l) => l.trim())
    .filter((l) => l.length >= 28 && /[A-Z<]{5}/.test(l));

  if (lines.length < 2) return null;

  // Try all formats in order of likelihood
  const parsers: Array<{ name: string; lineCount: number; parser: (lines: string[]) => MRZResult | null }> = [
    { name: "TD3", lineCount: 2, parser: parseTD3 },
    { name: "MRVA", lineCount: 2, parser: parseMRVA },
    { name: "TD2", lineCount: 2, parser: parseTD2 },
    { name: "MRVB", lineCount: 2, parser: parseMRVB },
    { name: "TD1", lineCount: 3, parser: parseTD1 },
  ];

  let bestResult: MRZResult | null = null;

  for (const { lineCount, parser } of parsers) {
    if (lines.length < lineCount) continue;

    // Try from each starting line
    for (let start = 0; start <= lines.length - lineCount; start++) {
      const subset = lines.slice(start, start + lineCount);
      try {
        const result = parser(subset);
