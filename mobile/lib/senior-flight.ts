...
function parseDurationToMinutes(raw?: string | null) {
  ...
  return (shortHours ? Number.parseInt(shortHours[1], 10) * 60 : 0)
    + (shortMinutes ? Number.parseInt(shortMinutes[1], 10) : 0);
}
...
function compareFlightByKind(kind: SeniorRecommendationKind, preferences: SeniorPreferences) {
...
export function buildSeniorRecommendations(flights: FlightOffer[], preferences: SeniorPreferences) {
  ...
  const defaultKind: SeniorRecommendationKind = preferences.priority === "fastest"
    ? "fastest"
  ...
  return {
    ...
    rankedFlights: baseRecommendations
      .map((item) => ({ ...item, kind: defaultKind }))
      .sort(compareFlightByKind(defaultKind, preferences)),
    fallbackApplied,
  };
}

export function formatCurrency(amount: number, currency = "USD", locale = "pt-BR") {
  try {
...
export function formatClock(dateTime?: string | null) {
  if (!dateTime) return "--:--";
...
export function formatShortDate(dateTime?: string | null) {
  if (!dateTime) return "";
...
export function getRouteLabel(flight: FlightOffer) {
  const firstSlice = flight.slices?.[0];
  if (firstSlice) {
    return `${firstSlice.originCode} - ${firstSlice.destinationCode}`;
  }

  return `${flight.originCode || "---"} - ${flight.destinationCode || "---"}`;
}

export function getSeniorBadge(kind: SeniorRecommendationKind) {
  if (kind === "comfort") {
    return {
      label: "Mais tranquilo",
      tone: "primary",
      description: "Bom para cansar menos e decidir com calma.",
    };
  }
  ...
  if (kind === "fastest") {
    return {
      label: "Mais rapido",
      tone: "success",
      description: "Menor tempo total de viagem entre as opcoes.",
    };
  }

  return {
    label: "Melhor equilibrio",
    tone: "warning",
    description: "Preco e conforto mais bem combinados.",
  };
}
